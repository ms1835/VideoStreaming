...
const Video = () => {
    ...
    const renderComments = (items, depth = 0) => {
        return items.map(comment => (
            <div key={comment._id} className="bg-white border border-slate-200 rounded-xl p-4 mb-4" style={{marginLeft: depth * 24}}>
                <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                        <div className="flex flex-wrap items-center justify-between gap-2">
                            <div>
                                <p className="font-semibold text-slate-900">{comment.creator?.name || comment.creator?.email || 'Unknown'}</p>
                        ...
                        <p className="mt-3 text-slate-800 whitespace-pre-line">{comment.content}</p>
    ...
    return (
        ...
        <div>
            ...
            <div className="px-6 py-2">
                ...
                <p className="text-gray-700 text-base truncate-multiline" title={currentVideo.description}>{currentVideo.description}</p>
            </div>
            <div className="flex flex-row justify-between px-6 py-1">
                <div className="flex justify-start items-center gap-2">
                    <p className="font-bold text-lg">{currentCreator?.name}</p>
                    <p className="text-gray-500 text-sm">{currentCreator?.subscribersCount || 0} subscriber</p>
                </div>
                <div className="flex justify-end items-center flex-wrap gap-2">
                    <span onClick={likeVideo} className={`${isLoggedIn ? "cursor-pointer" : ''} inline-flex bg-lime-200 rounded-full px-3 py-1 text-sm font-semibold text-gray-700`}> {currentVideo.likes}
                    ...
                    <span onClick={dislikeVideo} className={`${isLoggedIn ? "cursor-pointer" : ''} inline-flex bg-amber-200 rounded-full px-3 py-1 text-sm font-semibold text-gray-700`}> {currentVideo.unlikes}
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="ml-2 w-5 h-5">
            ...
            <div className="px-6 py-4 border-t border-slate-200">
                <div className="mb-4 flex flex-col gap-3">
                    ...
                    {isLoggedIn ? (
                        ...
                        <p className="text-sm text-slate-500">Sign in to add a comment.</p>
                    )}
                </div>
